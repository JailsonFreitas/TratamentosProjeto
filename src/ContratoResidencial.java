		...
		do {
			...
			do {
				try {
					...
					if (cliente != null && cliente.length() > 0) {
						...
						throw new Nadadigitado();
					}
				} catch (NullPointerException ex) {
					cancelar = true;
					saiu = true;
					break;
				} catch (Nadadigitado e) {
				}
			} while (ok == false);
			...
			do {
				try {
					...
					JOptionPane.showMessageDialog(null, "Este CPF já esta sendo ultilizado!");
				} catch (NullPointerException ex) {
					cancelar = true;
					saiu = true;
					break;
			...
			Object[] zonaEscolha = { "Urbana", "Sub urbana", "Zona Rural", "Cancelar" };
